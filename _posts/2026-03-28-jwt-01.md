...
Stateless really means what I thought: there is no session information stored in the server, instead
...
its issuer and thus you can trust it without contacting the issuer every time granted you already know
...
need for you to keep session information on the server. A new problem is introduced though: revocation.

Sidenote: in my past jobs people called many things stateless that weren't really.
Never assume that technical terms are used correctly in the software (any?) industry.

Back to revocation. If you don't store session info on the server and the token is the only thing carrying information
then it means that until the token is valid, it is valid. Even if it gets stolen. You can't revoke it. There is nothing to revoke
unless you store validity information on the server, but for every token that would be costly.

Enter refresh tokens, which are stateful because their validity is stored at the server - an important distinction.
...
The statelessness of truly stateless access tokens have some legit use cases too, typically for one-off
